...
conventions, and the Wikipedia page on _Quaternions and Spatial Rotations_
...
- C++ (Eigen)
  ...
  also offers the `coeffs()` method to access the underlying vector
  representation. These vectors are the **scalar-last**/`[x, y, z, w]` order.
  ...
  > individual components. Indeed, the `w()` method returns the 4th component,
  > not the 1st.

- Rust (nalgebra)

  In nalgebra, the `Quaternion` struct can be constructed from a 4-vector via
  the `from_vector` function, and the `coords` method can be used to access the
  ...
  **scalar-last**/`[x, y, z, w]` order.
  ...
  > nalgebra vectors offer `i`, `j`, `k`, and `w` methods to access the
  > individual components. Indeed, `w` returns the 4th component, not the 1st.
...
- C++ (Eigen)
  ...
  **scalar-first** order/`[w, x, y, z]`. This is the opposite order of the
  ...
  I contributed explicit factory functions to construct quaternions from
  individual components in an explicitly specified order to Eigen in
  [libeigen/eigen!1896](https://gitlab.com/libeigen/eigen/-/merge_requests/1896).
  These changes were merged in
  [Eigen 5.0](https://gitlab.com/libeigen/eigen/-/milestones/10#tab-merge-requests),
  and thus the following factories are also available:
...
- Rust (nalgebra)
  ...
  nalgebra inherited the component-wise constructor from Eigen, and thus also
  uses the **scalar-first** order/`[w, x, y, z]`.
...
- C++ (Eigen)
  ...
  The `toRotationMatrix()` method returns the corresponding rotation matrix.
